...
# Usage:
#   - In producer/consumer client, Run the thread in a daemon mode
...
class KafkaMetricsReporter(threading.Thread):
        ...
        try:
            ...
            for metric in vm_metrics_to_report:
                ...
                if self.report_to_vm and self.vm_up:
                    self.vm_post(self.session, reported_metric)
    ...
    '''
    def vm_post( self, session: requests.Session(), reported_metric: Dict[str, Any]) -> "request.Response().status_code":
        ...
        res=session.post(self.vm_rest_endpoint, headers=self.vm_rest_headers, data=data, timeout=self.vm_timeout_s)
